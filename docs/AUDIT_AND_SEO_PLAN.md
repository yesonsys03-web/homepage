...
> 작성일: 2026-03-03
> 대상: VibeCoder Playground 홈페이지
> 스택: React 19 + Vite 7 + FastAPI + PostgreSQL

---

## 목차

0. [현실 우선순위 재분류 (2026-03-03)](#0-현실-우선순위-재분류-2026-03-03)
1. [위험도별 수정 우선순위](#1-위험도별-수정-우선순위)
   - [🔴 CRITICAL — 즉시 수정 필수](#-critical--즉시-수정-필수)
   - [🟠 HIGH — 빠른 시일 내 수정](#-high--빠른-시일-내-수정)
   - [🟡 MEDIUM — 계획적으로 개선](#-medium--계획적으로-개선)
   - [🟢 LOW — 여유 있을 때 개선](#-low--여유-있을-때-개선)
2. [Google SEO 기획](#2-google-seo-기획)
3. [Naver SEO 기획](#3-naver-seo-기획)
4. [실행 체크리스트 (재분류 반영)](#4-실행-체크리스트-재분류-반영)
...
서비스 보안 또는 사용자 데이터에 직접 영향을 미치는 문제.
**배포 전 반드시 해결해야 합니다.**
...
- 상세 내용은 [Google SEO 기획](#2-google-seo-기획) 섹션 참고
...
VibeCoder Playground는 **바이브코딩 프로젝트 쇼케이스 플랫폼**입니다.
Google 검색에서 노출되기 위한 주요 타겟 키워드와 콘텐츠 전략을 정의합니다.
...
**단기 대안:** History API + SSG/SSR 도입 전까지는
URL 쿼리 파라미터 구조를 유지하되, canonical 태그로 중복 URL 문제 완화.

**장기 권장:** React Router v7 또는 Next.js로 마이그레이션 고려.
SSR을 통해 크롤러가 JavaScript 실행 없이 콘텐츠를 읽을 수 있게 됩니다.
...
네이버는 구글과 다른 별도 크롤러(Yeti)와 색인 알고리즘을 사용합니다.
국내 검색 점유율 약 60% (모바일 기준)를 차지하므로 별도 전략이 필요합니다.
...
네이버 검색은 **네이버 블로그 > 공식 사이트** 순으로 우선 노출하는 경향이 있습니다.
브랜드 키워드 검색 시 공식 블로그가 상단에 노출되도록 운영이 필요합니다.
...
- [ ] `react-helmet-async` 도입 + 화면별 동적 메타태그 구현
...
- 동적 메타태그(대체 적용): `src/App.tsx`에서 화면 상태별 `document.title`/`meta[name="description"]`를 직접 갱신하도록 적용. `react-helmet-async`는 React 19 환경 안정성 이슈로 보류하고, 런타임 안정성을 우선 확보하기 위함.
...
- [ ] FastAPI 동적 sitemap 엔드포인트 구현 (프로젝트 URL 포함)
- [ ] React Router 기반 독립 URL 구조 전환 검토/적용
- [ ] 프로젝트별 구조화 데이터 (`SoftwareApplication`) 추가
- [ ] FAQ/소개 페이지 JSON-LD 추가
- [ ] 네이버 블로그 채널 운영 시스템화
- [ ] 카카오 공유하기 버튼 구현
...
*이 문서는 2026-03-03 기준 코드 감사 결과를 바탕으로 작성되었습니다.*
*서비스 도메인 확정 후 URL 예시(`vibecoder.io`)를 실제 도메인으로 교체하세요.*
