...
NW|- list cards consistent with Home feed style
QB|- admin tab: 신고 큐, 댓글 블라인드/복구, 사용자 24시간 제한
YQ|- admin empty states and action buttons

Tone:
KB|- admin features should feel accessible, not hidden
VK|- clear action buttons for moderation actions
```

TT|## Screen 5 - Admin (Moderation)
```text
Create the Admin/Moderation screen for VibeCoder Playground.

Must include:
- report queue sorted by latest
- comment blind/restore actions
- 24-hour user restriction controls
- status indicators for each report (open, reviewing, resolved, rejected)
- batch action capabilities

UX requirements:
- quick action buttons (blind, restore, restrict)
- memo field for admin notes
- action confirmation for irreversible operations

Tone:
- functional and efficient, not playful
- clear visual hierarchy for queue management
```
