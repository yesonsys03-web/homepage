...
- 현재 어드민 구조는 문서의 목표 파일 구조와 거의 일치하며, `DataTable.tsx`는 별도 추가하지 않고 페이지별 테이블 구현으로 우선 반영했습니다.
...
현재 가드는 `currentScreen === 'admin'` 문자열 비교 방식(App.tsx:339)이라,
`/admin/users`, `/admin/content` 같은 중첩 라우트로 전환하면 **가드가 작동하지 않는다.**
...
App.tsx의 기존 `useEffect` 가드(`currentScreen === 'admin'`)는 중첩 라우트 전환 후 제거한다.

---

### 2. 대시보드 KPI 데이터 소스 확정
...
**결정 필요:** "이번 주 +47" 같은 증감 수치는 현재 API로 구현 불가.
→ 옵션 A: 백엔드에 `/api/admin/stats` 집계 엔드포인트 추가
→ 옵션 B: 증감 수치 없이 절댓값만 표시 (단순하고 빠름)
→ **권장: 옵션 B로 시작, 이후 필요 시 옵션 A 추가**
