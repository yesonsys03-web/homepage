...
### 18.6 숨김 퍼블리시 점검 로그 (About 블록)

운영자가 블록 숨김 동작을 실제 화면에서 확인했는지 아래 시트로 기록한다.

| date | environment | scenario | result | evidence | note |
| --- | --- | --- | --- | --- | --- |
| 2026-03-06 | local/dev | FAQ hidden -> publish -> About render hidden | pass | `uv run pytest tests/test_admin_page_editor_api.py::test_extract_about_content_omits_hidden_faq_block` | 서버 변환에서 `faq.visible=false`일 때 `faqs=[]` 검증 |
| 2026-03-06 | local/dev | Hero/Values/Team hidden -> publish -> About render hidden | pass | `uv run pytest tests/test_admin_page_editor_api.py::test_extract_about_content_omits_all_hidden_blocks` | 서버 변환에서 숨김 블록을 빈 payload로 매핑 검증 |
| 2026-03-06 | local/dev | About UI hidden payload render guard | pass | `pnpm test -- src/components/screens/AboutScreen.visibility.test.tsx` | Hero/Values/Team/FAQ/Contact 조건부 렌더 + FAQ JSON-LD 미생성 검증 |
| 2026-03-06 | staging/prod-like | 운영자 수동 점검(실브라우저) | pending | N/A | 릴리즈 직전 1회 이상 수동 캡처 필요 |

`운영 완료 조건`
- `staging/prod-like` 행에서 `result=pass` 1건 이상
- 캡처 또는 동영상 링크를 `evidence`에 기록
- 실패 시 18.2 게이트에서 `Hold`로 전환
